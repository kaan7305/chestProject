...
void Game::make_move(const Position &start, const Position &end) {
  ...
  if (end.first < 'A' || end.first > 'H' || end.second < '1' ||
      end.second > '8') {
  ...
  if (destOcc) {
    ...
    if (pieceChar == 'R' || pieceChar == 'r' || pieceChar == 'B' ||
        pieceChar == 'b' || pieceChar == 'Q' || pieceChar == 'q') {
      bool badShape =
  ...
  if (movedPiece) {
    ...
    if ((pieceChar == 'P' && end.second == '8') ||
        (pieceChar == 'p' && end.second == '1')) {
...
int Game::point_value(const bool &white) const {
  ...
  for (std::vector<std::pair<Position, const Piece *>>::const_iterator it =
    ...
    if (p != nullptr) { // add the value of the piece to the total
      total += p->point_value();
    }
  }
  return total;
